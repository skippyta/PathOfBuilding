...
while true do
  ...
  if not msg or type(msg) ~= 'table' then
    write_line({ ok = false, errorCode = 'INVALID_JSON', error = 'invalid json' })
    goto continue
  end
  ...
  if not handler then
    write_line({ ok = false, errorCode = 'UNKNOWN_ACTION', error = 'unknown action: '..tostring(action) })
    goto continue
  end
  local ok2, res = pcall(handler, params)
  if not ok2 then
    write_line({ ok = false, errorCode = 'HANDLER_EXCEPTION', error = 'exception: '..tostring(res) })
  else
    if type(res) ~= 'table' then
      res = { ok = false, errorCode = 'INVALID_HANDLER_RESPONSE', error = 'handler returned a non-object response' }
    elseif res.ok == false and not res.errorCode then
      res.errorCode = 'OPERATION_FAILED'
    end
    write_line(res)
